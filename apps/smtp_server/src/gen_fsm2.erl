...
%% %CopyrightEnd%
%%
%% Copied from gen_fsm.erl in OTP 15B release
%% Bill Barnhill <w.a.barnhill@gmail.com> modified in 2012-Feb to
%%    call state_change(OldStateName,NewStateName,State) on transition
%%    TODO// call state_change(none, InitialStateName, State) on start
%%    
%% NOTE: state_change not currently called when the next state
%% is overridden by the state_change callback. Solution is to either
%% loop while next state is different from current, repeatedly calling
%% state_change callback, or to make state_change only able to affect
%% state. Benefits to both, so one option is to make gen_fsm2a (
%% which loops, allowing overrides) and gen_fsm2b (which does not allow
%% overrides) - best solution, because we have to WabRule#2:Don't hoard 
%% the power, give it your users instead
...
%%%
%%%   state_change(OldStateName, NewStateName, StateData)
%%%
...
-callback state_change(OldStateName :: atom(), NewStateName :: atom(),
                       StateData :: term()) ->
    {next_state, NextStateName :: atom(), NewStateData :: term()} |
    {stop, Reason :: term(), NewStateData :: term()}.
...
stop({global, Name}) ->
    catch global:send(Name,'$gen_stop'),
    ok;
stop(Name) ->
    Name ! '$gen_stop',
    ok.

trigger_state_change({global, Name}, NStateName) ->
    catch global:send(Name, {'$gen_state_change', NStateName}),
    ok;
trigger_state_change(Name, NStateName) ->
    Name ! {'$gen_state_change', NStateName},
    ok.
...
init_it(Starter, Parent, Name0, Mod, Args, Options) ->
	...
	{ok, StateName, StateData} ->
	    ...
	    case Mod:state_change(none, StateName, StateData) of
		{next_state, NStateData} ->
		    loop(Parent, Name, StateName, NStateData, Mod, infinity, Debug);
		{stop, Reason, NStateData} ->
		    terminate(Reason, Name, init, Mod, StateName, NStateData, Debug)
	    end;
	{ok, StateName, StateData, Timeout} ->
	    proc_lib:init_ack(Starter, {ok, self()}), 	    
	    case Mod:state_change(none, StateName, StateData) of
		{next_state, NStateData} ->
		    loop(Parent, Name, StateName, NStateData, Mod, Timeout, Debug);
		{stop, Reason, NStateData} ->
		    terminate(Reason, Name, init, Mod, StateName, NStateData, Debug)
	    end;
	{stop, Reason} ->
...
handle_msg(Msg, Parent, Name, StateName, StateData, Mod, _Time) -> %No debug here
    From = from(Msg),
    case dispatch(Msg, Mod, StateName, StateData) of
	{next_state, NStateName, NStateData} ->
	    if 
		StateName =/= NStateName ->
		    case Mod:state_change(StateName, NStateName, NStateData) of
			{next_state, NStateDataFinal} ->
			    loop(Parent, Name, NStateName, NStateDataFinal, Mod, infinity, []);
			{stop, Reason, NStateDataFinal} ->
			    terminate(Reason, Name, Msg, Mod, NStateName, NStateDataFinal, [])
		    end;
		true ->
	...
	{next_state, NStateName, NStateData, Time1} ->
		...
		StateName =/= NStateName ->
		    case Mod:state_change(StateName, NStateName, NStateData) of
			{next_state, NStateDataFinal} ->
			    loop(Parent, Name, NStateName, NStateDataFinal, Mod, Time1, []);
			{stop, Reason, NStateDataFinal} ->
			    terminate(Reason, Name, Msg, Mod, NStateName, NStateDataFinal, [])
		    end;
		true ->
...
handle_msg(Msg, Parent, Name, StateName, StateData, Mod, _Time, Debug) -> 
	...
	{next_state, NStateName, NStateData} ->
		...
		StateName =/= NStateName ->
		    case Mod:state_change(StateName, NStateName, NStateData) of
			{next_state, NStateDataFinal} ->
			    loop(Parent, Name, NStateName, NStateDataFinal, Mod, infinity, Debug1);
			{stop, Reason, NStateDataFinal} ->
			    terminate(Reason, Name, Msg, Mod, NStateName, NStateDataFinal, Debug)
		    end;
		true ->
	...
	{next_state, NStateName, NStateData, Time1} ->
		...
		StateName =/= NStateName ->
		    case Mod:state_change(StateName, NStateName, NStateData) of
			{next_state, NStateDataFinal} ->
			    loop(Parent, Name, NStateName, NStateDataFinal, Mod, Time1, Debug1);
			{stop, Reason, NStateDataFinal} ->
			    terminate(Reason, Name, Msg, Mod, NStateName, NStateDataFinal, Debug)
		    end;
		true ->
...
dispatch('$gen_stop', Mod, StateName, StateData) ->
    terminate(normal, self(), terminating, Mod, StateName, StateData, []);

dispatch({'$gen_state_change', NStateName}, Mod, StateName, StateData) ->
    if 
	StateName =/= NStateName ->
	    case Mod:state_change(StateName, NStateName, StateData) of
		{next_state, NStateData} -> {next_state, NStateName, NStateData};
...
terminate(Reason, Name, Msg, Mod, StateName, StateData, Debug) ->
    io:format("GOT HERE~n-----------~n"),
    case catch Mod:state_change(StateName, none, StateData) of
	Result ->
	    io:format("GOT HERE 2 ---~n~p~n-----------~n", [Result])
    end,
    io:format("GOT HERE 3 ---~n~p~n-----------~n", [Result]),
